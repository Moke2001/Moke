...
class FermionicCode:
    ...
    def __init__(self):
        ...
        self.gauge_list=[]
        code.check_stable_list = []
        code.check_gauge_list = []
        code.check_measure_list = []
    ...
    def get_core_matrix(self):
        GF=galois.GF(2**1)
        matrix=GF(np.zeros((self.number_checker//2,self.number_qubit),dtype=int))
        for i in range(len(self.check_list)):
            temp_x=self.check_list[i].x_vector
            if len(temp_x)>0:
                matrix[i,temp_x]=1
    ...
    def get_minimal_logical_operator(self,weight,number):
        ...
        for each in itertools.combinations(pos, weight):
            ...
            temp[list(each)] = 1
            case=np.count_nonzero(matrix @ temp)
            print(case)
            if case == 0:
    ...
    def get_distance(self):
        ...
        for i in range(len(self.gauge_list)):
            ...
            temp_x = self.gauge_list[i].x_vector
            temp_z = self.gauge_list[i].z_vector
            temp[temp_x * 2] = 1
            temp[temp_z * 2 + 1] = 1
            gauge_list.append(temp)
        return QuantumCodeDistance(self.check_list, gauge_list,self.number_qubit)


if __name__ == '__main__':
    code=FermionicCode()
    code.linear_combine(BicycleCode(100,10,40,741))
    code.get_distance()
