...
if __name__=='__main__':
    ...
    code_memory.linear_combine(ProjectiveCode(2))

    opx=MajoranaOperator([0,1,3,4,7],[],1)
    opz=MajoranaOperator([],[0,1,3,4,7],1)
    code_memory.logical_operator_list_x.append(opx)
    code_memory.logical_operator_list_z.append(opz)
    n_memory = code_memory.number_qubit  # 内存的粒子数
    code=ZechuanLatticeSurgery(code_memory,0)
    stabilizer_list=code.check_stable_list+code.check_fixed_list+code.check_gauge_list
    gauge_list=code.check_origin_list+code.check_measure_list+code.ancilla_list
    print(QuantumCodeDistance(stabilizer_list,gauge_list,code.number_qubit))